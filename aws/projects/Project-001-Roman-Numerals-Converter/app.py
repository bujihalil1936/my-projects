...
def main_post():
    if request.method == 'POST':
        ...
        if not alpha.isdecimal():
            return render_template('index.html', developer_name='HALİL-KARADAYİ', not_valid=True)
        number=int(alpha)
        if not 0 < number < 4000:
            return render_template('index.html', developer_name='HALİL-KARADAYİ', not_valid=True)
        return render_template('result.html', number_decimal=number, number_roman=convert(number), developer_name='HALİL-KARADAYİ')
    else:
        return render_template('index.html', developer_name='HALİL-KARADAYİ', not_valid=False)
